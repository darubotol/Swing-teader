...
def main():
    ...
    # 6b. Trade log — update existing open trades, then log today's new ones ---
    log = trade_log.load_log()
    log = trade_log.update_open_trades(log, universe_data, datetime.date.today().isoformat())
    log = trade_log.add_new_trades(log, buckets, datetime.date.today().isoformat())
    ...
    html_page = html_report.build_html(regime, sector_ranking, buckets, data_ts, trade_stats=stats)
