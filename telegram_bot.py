    ...
    for chat_id in config.TELEGRAM_CHAT_IDS:
        for chunk in chunks:
            ...
            if resp.status_code != 200:
                print(f"[telegram_bot] Failed to send message to {chat_id}: {resp.status_code} {resp.text}")
...
def send_document(filepath: str, caption: str = "") -> None:
    ...
    for chat_id in config.TELEGRAM_CHAT_IDS:
        ...
        if resp.status_code != 200:
            print(f"[telegram_bot] Failed to send document to {chat_id}: {resp.status_code} {resp.text}")
