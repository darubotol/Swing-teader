...
def full_report_markdown(regime: dict, sector_ranking: list, buckets: dict, data_ts: str) -> str:
    ...
    if regime["regime"] == "RED":
        decision = "D. NO TRADE — market conditions unsuitable."
    elif n_actionable == 0:
        decision = "C. WATCHLIST ONLY" if buckets["watch"] else "D. NO TRADE"
    elif n_actionable == 1:
    ...
    lines.append(
        ...
        "liquidity, corporate events and order details before trading. Fundamental "
        "and news-based checks are not automated in this pipeline — confirm manually."
        "_"
    )
    return "\n".join(lines)
