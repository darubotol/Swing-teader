...
def analyze_stock(ticker: str, df: pd.DataFrame, nifty_df: pd.DataFrame) -> dict:
    ...
    # Extension filter: skip entries chasing a stock that's already run too far
    # above its 50-DMA. Buying extended moves is a classic cause of low win
    # rates in breakout systems (little room left before mean-reversion).
    extension_from_dma50 = (close - dma50) / dma50 if dma50 else 0
    is_extended = extension_from_dma50 > 0.12

    setup = None
    entry = stop = target = None
    structure_notes = []

    # Volume threshold raised from 1.3x to 1.6x — requires stronger conviction
    # before treating a breakout as real, filtering out noisy/weak breakouts.
    breakout_today = close > high_20 and last_vol > 1.6 * avg_vol_20
    near_resistance_retest = (
        high_20 * 0.97 <= close <= high_20 * 1.02
        and df["Close"].iloc[-5:-1].max() > high_20  # broke out within last few days
    )
    # Trend Pullback now also requires the 50-DMA to be structurally above the
    # 200-DMA (not just price above 200-DMA) — a stronger confirmation that
    # this is an established uptrend, not a fragile/early one.
    strong_structural_trend = (dma200 is not None) and (dma50 > dma200)
    pullback_to_support = (
        above_50 and dma50_rising and strong_structural_trend
    ...
    if is_extended:
        return None  # too far above the 50-DMA — don't chase it

    if breakout_today and consolidation_range < 0.18:
        setup = "Breakout"
        entry = round(high_20 * 1.003, 2)  # confirmation buffer above resistance
        # Stop widened from 1.5x to 2.0x ATR — 1.5x was getting shaken out by
        # ordinary volatility before the move had room to work.
        stop = round(min(low_20, close - 2.0 * atr), 2)
        ...
        target = round(entry + 2.2 * (entry - stop), 2)
        structure_notes.append(f"Pullback toward {'20' if support_level==dma20 else '50'}-DMA in an established uptrend (50-DMA above 200-DMA).")
    else:
    ...
    return {
        ...
        "recent_closes": [round(float(c), 2) for c in df["Close"].tail(40).tolist()],
    }
...
def score_candidate(stock_info: dict, regime: dict, sector_rank: int, num_sectors: int) -> dict:
    ...
    if stock_info["setup"] == "Breakout":
        vol_pts = w["volume_confirmation"] if vol_ratio >= 1.6 else w["volume_confirmation"] * (vol_ratio / 1.6)
    else:
